...
## 2026-03-25 - 修复港澳 inset 地图不显示颜色

### 修复
- **根因**：港澳在 cityData 只有整体条目（810000/820000），而用户在省级视图点击的是区级 feature（810001-810018 / 820001-820008），这些 adcode 不在 CITIES_BY_PROVINCE 里，导致 `getProvinceMaxLevel` 始终返回 0
- **修复**：将香港 18 区、澳门 8 堂区逐一加入 cityData，城市总数 436 → 462

## 2026-03-25 - 导出图片提升至 4800×3200

### 优化
- `exportImage.ts`：新增 `SCALE = 2`，Canvas 物理尺寸翻倍（4800×3200），SVG 同步按 SCALE 渲染，overlay 文字/图例通过 `ctx.scale()` 同比放大，无需修改坐标逻辑

## 2026-03-25 - 台湾省县市级支持 + 样式优化

### 新增
- **台湾省 22 县市级地图**：GeoJSON 来源 g0v/twgeojson（WGS84），含台北/新北/台中/高雄等 19 县市 + 澎湖/金門/連江（马祖）3 离岛
- 坐标处理：Douglas-Peucker（epsilon=0.01）简化 + 过滤极小岛屿（MIN_AREA=0.0001），文件从 8MB 压缩到 56KB
- g0v 数据原始即为 CW（顺时针），无需反转（与 DataV 相反），详见 `docs/bugfix-geojson-winding-order.md`
- `cityData.ts` 从 415 增加到 436 个城市（+21 台湾县市）
- `shareCodec.ts` 注释更新为 436 城市

### 样式优化
- 省级视图导航栏：左侧改为面包屑（全国 › 省名），右侧放返回全国按钮
- 反馈/Issue 按钮移入右侧面板底部，左对齐，移除之前的悬浮定位

## 2026-03-25 - 台湾省支持

### 修复
- `ProvinceMap`：过滤后 features 为空时（如台湾省 GeoJSON 只有省级轮廓），回退显示省级轮廓作为整体可点击区域
- 台湾省现可在省级视图中点击标记访问等级，与其他省保持一致
